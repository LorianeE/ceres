...
describe("ShoppingListService", () => {
  ...
  describe("getItem()", () => {
    it("should get item to shoppingList if found", async () => {
      // GIVEN
      const shoppingListModel = {
        findById: jest.fn().mockReturnValue({
      ...
      // WHEN
      const result = await shoppingListService.getItem("1234", "itemId");

      // THEN
      expect(result).toEqual({_id: "itemId"});
      expect(shoppingListModel.findById).toHaveBeenCalledTimes(1);
      expect(shoppingListModel.findById).toHaveBeenCalledWith("1234");
    });
    it("should return null if item not found in shoppingList", async () => {
      // GIVEN
      const shoppingListModel = {
        findById: jest.fn().mockReturnValue({
          exec: jest.fn().mockResolvedValue({
      ...
      // WHEN
      const result = await shoppingListService.getItem("1234", "itemId2");
    ...
    it("should return null if shoppingList not found", async () => {
      // GIVEN
      const shoppingListModel = {
        findById: jest.fn().mockReturnValue({
