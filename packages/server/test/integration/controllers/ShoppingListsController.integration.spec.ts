...
describe("ShoppingLists", () => {
  ...
  describe("Adding a shoppinglist to DB and retrieve it", () => {
    ...
    it("should create new shopping list", async () => {
      ...
      const responsePost = await request.post(`/rest/users/${userId}/shopping-list`).send(shoppingList).expect(201);
      shoppingListId = responsePost.body.id;
    });
    it("should retrieve new shopping list", async () => {
      // THEN RETRIEVE IT
      newShoppingListResponse = await request.get(`/rest/shopping-lists/${shoppingListId}`).expect(200);
    ...
    it("should update the shopping list", async () => {
      ...
      const responsePut = await request.put(`/rest/shopping-lists/${shoppingListId}`).send(updatedList).expect(200);
